...
impl SqliteStore {
    /// Open/create schema v2, atomically migrating validated v1 state. Foreign application IDs and nonempty unknown databases
    ...
    pub fn open(path: impl AsRef<Path>, scope: NetworkScope) -> Result<Self> {
        ...
        if app == 0 && version == 0 && objects == 0 {
            ...
            backup_state::validate_native_schema_version(&tx, 1)?;
            tx.execute_batch(payment::PAYMENT_SCHEMA)?;
            tx.pragma_update(None, "user_version", VERSION)?;
