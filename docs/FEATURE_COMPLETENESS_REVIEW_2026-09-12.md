...
| FC10 provider/browser | Typed conversion/wrapper reads, bounded multi-address outpoints and inclusive delta queries, bounded WS reconnect/canonical header replay; browser Fetch and bounded window/worker WebSocket adapters; passive account/chain/disconnect revisions, verified exact transaction signing, explicit signed submission, observed wallet-mediated sends and bounded browser receipt waiting with portable canonicality checks; atomic scoped IndexedDB snapshots and durable HD and payment-code address allocation with backup floors and explicit restart/cancellation recovery | Remaining reference RPC mapping, complete browser wallet-state integration and real extension interoperability remain open |
| FC11 usability/parity | Runnable examples, workflow guide, updated semantic mappings with tests/deviations; bounded readable ABI import/full/minimal formatting and lossless JSON metadata export; JS regeneration wired into CI | Full declaration/overload reconciliation and additional end-to-end examples remain open; rows are not blanket marked complete |
