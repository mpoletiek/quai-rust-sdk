...
pub use replacements::QuaiReplacement;
...
impl KeyOrigin {
    ...
    fn decode(bytes: &[u8]) -> Result<Self> {
        ...
        let account = u32::from_be_bytes(array(&bytes[3..7])?);
        let index = u32::from_be_bytes(array(&bytes[7..11])?);
...
impl SqliteStore {
    /// Open/create schema v3, atomically migrating validated v1/v2 state. Foreign application IDs and nonempty unknown databases
    ...
    pub fn open(path: impl AsRef<Path>, scope: NetworkScope) -> Result<Self> {
        ...
        } else if app == APP_ID && matches!(version, 1 | 2) {
