...
2. Call `QiChangePool::allocate`. It first takes addresses an earlier pool released, lowest index first, then reserves raw derivation ranges, derives same-zone Qi addresses on the change branch and persists their public metadata **before returning addresses**. The opaque pool cannot be cloned, imported or reconstructed. Preparation requires its persisted address metadata and account derivation cursor in the receiving store; matching only the network scope is insufficient. A reserved range stays consumed after failure, cancellation or process death, and so does a pool dropped without `release`.
3. Refresh the wallet's qualified discovery snapshot **after allocation**. Adding change metadata invalidates the previous checkpoint; a pool made only of released addresses adds none and needs no refresh. A complete discovery commit must cover existing metadata, including the new addresses. `snapshot()` returning no checkpoint is not spendable. A manually constructed `Snapshot` remains a caller/source claim, not a proof.
...
5. Review `PreparedQiTransaction::transaction()`, `fee()`, `recipient_outputs()` and `signing_digest()`. Recipient outputs appear first; change follows. No mutation API is exposed. If the user rejects it, `change.reclaim(&mut store, prepared)` releases the unsigned reservation and returns its change to the pool; `reclaim_operation` does the same for a conversion or wrapping. Dropping a prepared transaction keeps its unsigned reservation until explicit `release_unsigned(id)`, and its change stays burned.
...
// change.reclaim(&mut store, prepared)?;
...
// When the wallet is done with the pool, store its unused addresses for the
// next allocate instead of burning them.
change.release(&mut store)?;
```
