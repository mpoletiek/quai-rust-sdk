...
Call `unsubscribe` to remove an individual subscription while keeping the connection. Dropping an active subscription or cancelling an in-flight registration/unsubscribe terminates the shared session, since either could leave a subscription running on the node unseen. A subscription that overflows its queue or the shared notification byte budget ends alone: its receiver yields `SubscriptionLagged`, the session unsubscribes it on the node, and other subscriptions and requests continue. After a terminal error, `recv` returns `None`, and dropping or unsubscribing the ended subscription is harmless. `WsConfig::ping_interval` (30 seconds by default) pings an idle connection and closes the session as disconnected after two intervals with no inbound frame, so a half-open connection fails instead of going silent. Graceful `shutdown` is bounded by the request deadline and aborts the session task if it cannot finish. Last-owner drop also aborts the task. To recover, create a new connection and reconcile application state before resubscribing; no transparent replay occurs.

## Validation

The real loopback tests in `tests/websocket.rs` cover path/query preservation, exact endpoint checks, reversed concurrent responses, immediate notifications, exact pending/log arguments, graceful unsubscribe, notification count/byte overflow ending only the lagging subscription, keepalive detection of a silent peer, request cancellation and late responses, registration cancellation, pending-request shutdown, deadlines including permit waits, inbound/outbound size limits, unsolicited/duplicate malformed envelopes and refused redirects. The opt-in live test checks chain ID, subscribes to `newHeads`, receives a header notification, unsubscribes and closes; it never sends transactions.
