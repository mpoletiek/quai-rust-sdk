...
| FC03 durable lifecycle | Unified persisted signed Qi operation types, restart/rebroadcast, exact canonical inclusion reconciliation, atomic head-replay rollback and generation-fenced observations, unsigned nonce recovery and explicit gap repair | Qi same-input candidate graphs and v5 backups are implemented; terminal claim release and full wallet-state application of canonical replay and fault/soak qualification remain open; absence never authorizes claim reuse |
