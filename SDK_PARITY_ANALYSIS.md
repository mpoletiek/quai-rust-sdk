...
| `deviation` | 1059 | Documented replacement, stricter behavior, correction or omission |
| `partial` | 168 | A mapping exists with unfinished behavior or scope |
| `pending` | 2639 | No completed row-level reconciliation; not proof of absence |
...
| Quai transactions | Native durable prepare/review/sign/broadcast, nonce reservation, access lists, deployment and cross-zone intents | Native SQLite sessions and the browser ordinary account session use explicit pending/latest policy; browser conversion/deployment/replacement integration remains open |
| Qi transactions | Native durable selection/fee convergence/preparation/signing/submission; exact special-operation custody | Full browser Qi preparation/recovery remains absent; ordinary browser Quai account preparation/signing/submission is now integrated |
...
| Quai → Qi | Explicit native conversion preparation, simulation/fees/slippage, signed custody and destination observations | Quote history depends on node behavior; per-operation maturity/production execution need stronger evidence |
...
| Browser persistence | Durable allocation, account/Qi custody, consistent backup capture and atomic multi-journal live restore | Explicit enumeration; initialized targets required by the coordinator; ordinary account fee/prepare/sign/root-submit orchestration now exists; full Qi/specialized recovery and history compaction remain open |
...
| --- | --- | --- |
| Complete browser wallet sessions | Custody, discovery, atomic recovery and ordinary account preparation/submission exist; complete browser Qi and specialized account preparation/recovery still need integration | A browser workflow that composes policy, exact preparation, durable authorization/submission and reconciliation, with interruption/concurrency tests |
...
| `ContractTransactionResponse` | 58 | 0 | 0 | 0 |
| `QiHDWallet` | 30 | 28 | 6 | 28 |
| `QuaiTransactionResponse` | 58 | 0 | 0 | 0 |
