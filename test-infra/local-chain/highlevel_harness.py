...
p=argparse.ArgumentParser(description=__doc__);p.add_argument('command',choices=['build','run']);p.add_argument('--mode',choices=['init-fund','account-prepare','account-broadcast','qi-prepare','qi-broadcast','verify','verify-qi'])
a=p.parse_args()
...
(CLIENT/'src').mkdir(parents=True,exist_ok=True);shutil.copy2(HERE/'highlevel_client.rs',CLIENT/'src/main.rs')
